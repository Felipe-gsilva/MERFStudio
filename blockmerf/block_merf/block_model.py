...
def skimage_ssim(image, rgb):
    ...
    values = [
        structural_similarity(
            gt, img, win_size=11, multichannel=True, channel_axis=2, data_range=1.0
        )
        for gt, img in zip(
...
from merf.baking.baking_utils import (
    as_mib,
    get_atlas_block_size,
    parallel_write_images,
    reshape_into_3d_atlas_and_compute_indirection_grid,
    save_8bit_png,
    save_json,
)
...
def get_chunk_idx(points, chunks_boundary):
    ...
    for lod_idx, lod in enumerate(chunks_boundary):
        for chunk_idx, boundary in enumerate(lod):
            ...
            mask_min = (points[:, :2] >= boundary_tensor[:, :2]) | (
                boundary_tensor[:, :2] == float("-inf")
            )
            mask_max = (points[:, :2] <= boundary_tensor[:, 2:]) | (
                boundary_tensor[:, 2:] == float("inf")
            )
...
def get_chunk_boundary_and_transform(
    num_chunks: List[int] = [1, 4], global_boundary=None, num_xy: List[List[int]] = None
) -> Tuple(List[List[List[float]]], List[List[Tensor]]):
    """
    ...
    for lod in range(len(num_chunks)):
        ...
        for i in range(num_xy[lod][0]):
            for j in range(num_xy[lod][1]):
                side_length_x = (
                    global_boundary[0][1] - global_boundary[0][0]
                ) / num_xy[lod][0]  #  [-1, 1] length is 2
                side_length_y = (
                    global_boundary[1][1] - global_boundary[1][0]
                ) / num_xy[lod][1]  #  [-1, 1] length is 2
                x_min = (
                    (global_boundary[0][0] + i * side_length_x)
                    if i > 0
                    else float("-inf")
                )
                x_max = (
                    (global_boundary[0][0] + (i + 1) * side_length_x)
                    if i < int(num_xy[lod][0]) - 1
                    else float("inf")
                )
                y_min = (
                    global_boundary[1][0] + j * side_length_y
                    if j > 0
                    else float("-inf")
                )
                y_max = (
                    global_boundary[1][0] + (j + 1) * side_length_y
                    if j < int(num_xy[lod][1]) - 1
                    else float("inf")
                )
...
class BlockModelConfig(ModelConfig):
    ...
    global_boundary: List[List[float]] = field(
        default_factory=lambda: [[-0.95, 0.95], [-0.95, 0.95]]
    )
    ...
    def __post_init__(self):
        ...
        with open("data.json", "w") as json_file:
            json.dump(
                [
                    self.chunks_transform[0][i].tolist()
                    for i in range(len(self.chunks_transform[0]))
                ],
                json_file,
            )
    ...
    def init_chunk_config(self):
        ...
        self.chunks_boundary, self.chunks_transform = get_chunk_boundary_and_transform(
            self.num_chunks, global_boundary=self.global_boundary, num_xy=self.num_xy
        )
        self.chunks_optimized = [
            [True for i in range(num_chunk)] for num_chunk in self.num_chunks
        ]
        if self.chunks_model_config is None:
            self.chunks_model_config = []
            for num_chunk in self.num_chunks:
                self.chunks_model_config.append(
                    [
                        MERFModelConfig(
                            use_appearance_embedding=self.use_appearance_embedding
                        )
        ...
        block_config.update(
            {
                "chunks_boundary": self.chunks_boundary,
                "chunks_transform": [
                    [chunk_transform.tolist() for chunk_transform in lod]
                    for lod in self.chunks_transform
                ],
...
def sparsity_loss(random_positions, random_viewdirs, density, voxel_size):
    step_size = stepsize_in_squash(random_positions, random_viewdirs, voxel_size)
...
class BlockMERFProposalNetworkSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        for i_level in range(n + 1):
            ...
            if is_prop:
                samples_chunk_idx = get_chunk_idx(
                    ray_samples.frustums.get_gaussian_samples(), chunks_boundary
                )
                density = torch.zeros(ray_samples.shape).to(ray_samples.deltas)[
                    ..., None
                ]
                    ...
                    for j, chunk_density_fn in enumerate(density_fns[lod]):
                        ...
                        if sub_samples.shape[0] > 0:
                            if updated:
                                ...
                                sub_density, _ = chunk_density_fn[i_level](
                                    sub_samples,
                                    samples_transform=chunks_transform[lod][j],
                                )
                            else:
                                with torch.no_grad():
                                    # density = density_fns[i_level](ray_samples.frustums.get_positions())
...
class BlockModel(Model):
    ...
    def populate_modules(self):
        ...
        for lod_idx in range(self.config.num_lods):
            ...
            for i in range(self.config.num_chunks[lod_idx]):
                self.config.chunks_model_config[lod_idx][
                    i
        ...
        self.direction_encoding = MERFViewEncoding(
            in_dim=3, deg_enc=4, include_input=True
        )
        self.chunks_transform = [
            [chunks.to(self.kwargs["device"]) for chunks in lod]
            for lod in self.config.chunks_transform
        ]
        # Collider
        ...
        if self.config.bounded_scene:
            self.collider = AABBBoxCollider(
                scene_box=self.scene_box, near_plane=self.config.near_plane
            )
        else:
        ...
        for lod_idx in range(self.config.num_lods):
            self.proposal_networks.append(
                torch.nn.ModuleList(
                    self._model[lod_idx][i].proposal_networks
                    for i in range(self.config.num_chunks[lod_idx])
                )
            )
        ...
        self.density_fns.extend(
            [
                [
                    [network.get_density for network in chunks_network]
                    for chunks_network in lod_network
                ]
                for lod_network in self.proposal_networks
            ]
        )
        ...
        self.s3im_loss = S3IM(
            patch_height=self.config.patch_h, patch_width=self.config.patch_w
        )
        # metrics
    ...
    def get_training_callbacks(
        ...
        def set_alpha_culling_value(step):
            ...
            if step < self.config.alpha_threshold_param[0]:
                ...
                t = (step - self.config.alpha_threshold_param[4]) / (
                    self.config.alpha_threshold_param[1]
                    - self.config.alpha_threshold_param[4]
                )
                alpha_thres = log_lerp(
                    t,
                    self.config.alpha_threshold_param[2],
                    self.config.alpha_threshold_param[3],
                )
            else:
                t = (step - self.config.alpha_threshold_param[0]) / (
                    self.config.alpha_threshold_param[4]
                    - self.config.alpha_threshold_param[0]
                )
                alpha_thres = t * self.config.alpha_threshold_param[2]
            for lod_idx in range(self.config.num_lods):
                for chunk_idx in range(self.config.num_chunks[lod_idx]):
                    self._model[lod_idx][chunk_idx].field.set_alpha_threshold(
                        alpha_thres
                    )
    ...
    def get_baking_outputs_for_camera_ray_bundle(
        self, ray_bundle: RayBundle, lod_idx=None, chunk_idx=None
    ):
        ...
        ray_samples, weights_list, ray_samples_list = self.proposal_sampler(
            ...
            chunks_transform=self.chunks_transform,
            chunks_boundary=self.config.chunks_boundary,
            num_lods=self.config.num_lods,
        )
        ...
        samples_chunk_idx = get_chunk_idx(
            ray_samples.frustums.get_gaussian_samples(),
            self.config.chunks_boundary,
            chunk_idx,
        )
        ...
        # for i in range(self.config.num_lods):
        density = torch.zeros(
            ray_samples.shape + (1,), device=self.device, dtype=torch.float32
        )
        ...
        sub_samples = ray_samples[chunk_mask]
        if sub_samples.shape[0] > 0:
            sub_field_outputs = chunk_model.field.get_outputs(
                sub_samples,
                sub_samples.frustums.directions,
                samples_transform=self.chunks_transform[lod_idx][chunk_idx],
            ...
            for j, chunk_model_j in enumerate(self._model[lod_idx]):
                if j != chunk_idx:
                    ...
                    if sub_samples_j.shape[0] > 0:
                        sub_field_outputs = chunk_model_j.field.get_outputs(
                            sub_samples_j,
                            sub_samples_j.frustums.directions,
                            samples_transform=self.chunks_transform[lod_idx][j],
                            appearance_embedding=self.appearance_embedding,
                        )
                        density[chunk_mask_j] = sub_field_outputs[
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        if self.config.global_deferred:
            rgb_global_list = []
        loss_specular_sum = 0.0
        loss_specular_min = 0.0
        ...
        dir_encode = self.direction_encoding(ray_bundle.directions).to(
            samples_pos.device
        )

        for i in range(self.config.num_lods):
            density = torch.zeros(
                ray_samples.shape + (1,), device=samples_pos.device, dtype=torch.float32
            )
            features = torch.zeros(
                ray_samples.shape + (7,), device=samples_pos.device, dtype=torch.float32
            )
            chunk_rgb = torch.zeros(
                (ray_samples.shape[0], self.config.num_chunks[i], 3),
                device=samples_pos.device,
                dtype=torch.float32,
            )
            chunk_rgb_global = torch.zeros(
                (ray_samples.shape[0], self.config.num_chunks[i], 3),
                device=samples_pos.device,
                dtype=torch.float32,
            )
            chunk_weights = torch.zeros(
                (ray_samples.shape[0], self.config.num_chunks[i], 1),
                dtype=torch.float32,
                device=samples_pos.device,
            )
            # specular_chunk_list = []
            for j, chunk_model in enumerate(self._model[i]):
                ...
                if sub_samples.shape[0] > 0:
                    sub_field_outputs = chunk_model.field.get_outputs(
                        sub_samples,
                        sub_samples.frustums.directions,
                        samples_transform=self.chunks_transform[i][j],
                        appearance_embedding=self.appearance_embedding,
                    )
                    density[chunk_mask] = sub_field_outputs[FieldHeadNames.DENSITY]
                ...
                # simulate volume rendering inside chunk
                chunk_samples_density = torch.where(
                    chunk_mask.unsqueeze(-1).expand_as(density),
                    density,
                    torch.zeros_like(density),
                )
                chunk_samples_features = torch.where(
                    chunk_mask.unsqueeze(-1).expand_as(features),
                    features,
                    torch.zeros_like(features),
                )
                chunk_samples_weights = ray_samples.get_weights(chunk_samples_density)
                chunk_ray_features = self.renderer_features(
                    features=chunk_samples_features, weights=chunk_samples_weights
                )  # BxC

                if self.config.unique_deferred:
                    chunk_ray_specular = self._model[i][0].deferred_mlp(
                        torch.cat([chunk_ray_features, dir_encode], dim=-1)
                    )
                else:
                    chunk_ray_specular = chunk_model.deferred_mlp(
                        torch.cat([chunk_ray_features, dir_encode], dim=-1)
                    )
                ...
                chunk_rgb[:, j, :] = chunk_ray_rgb  # Bx3
                if self.config.global_deferred:
                    chunk_ray_specular_global = self.global_deferred_mlp(
                        torch.cat([chunk_ray_features, dir_encode], dim=-1)
                    )
                    chunk_ray_rgb_global = (
                        chunk_ray_features[..., 0:3] + chunk_ray_specular_global
                    )
                ...
                if self.training and self.config.specular_mult > 0:
                    ...
                    rgb_specular_ = chunk_model.deferred_mlp(
                        torch.cat([chunk_ray_features, dir_encode_], dim=-1)
                    )
                    ...
                    loss_specular = (
                        torch.cosine_similarity(eps_dir, dir_ori, dim=-1).unsqueeze(-1)
                        * (rgb_specular_ - rgb_specular_ori) ** 2
                    ).mean()
            ...
            chunk_weights = chunk_weights.scatter_add(
                1, samples_chunk_idx[..., i].unsqueeze(-1).to(torch.int64), weights_real
            )
        ...
        for i in range(self.config.num_proposal_iterations):
            for lod in range(self.config.num_lods):
                outputs[f"prop_depth_{lod}_{i}"] = self.renderer_depth(
                    weights=weights_list[lod][i], ray_samples=ray_samples_list[lod][i]
                )
        return outputs
    ...
    def get_metrics_dict(self, outputs, batch):
        ...
        for lod in range(self.config.num_lods):
            ...
            metrics_dict[f"psnr_global_{lod}"] = (
                self.psnr(
                    outputs["rgb_global"][lod],
                    image.to(outputs["rgb_global"][lod].device),
                )
                if self.config.global_deferred
                else 0.0
            )
            if self.training:
                metrics_dict[f"distortion_{lod}"] = distortion_loss(
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        for lod in range(self.config.num_lods):
            loss_dict[f"rgb_loss_{lod}"] = self.rgb_loss(
                image.to(outputs["rgb"][lod].device), outputs["rgb"][lod]
            )
            # print(outputs["rgb"][lod].shape)
            loss_dict[f"s3im_loss_{lod}"] = self.s3im_loss(
                outputs["rgb"][lod], image.to(outputs["rgb"][lod].device)
            )
            # print(outputs["rgb"][lod].shape)
            loss_dict[f"rgb_global_loss_{lod}"] = (
                self.rgb_loss(
                    image.to(outputs["rgb_global"][lod].device),
                    outputs["rgb_global"][lod],
                )
        ...
        if self.training:
            for lod in range(self.config.num_lods):
                loss_dict[f"interlevel_loss_{lod}"] = (
                ...
                loss_dict[f"distortion_loss_{lod}"] = (
                    self.config.distortion_loss_mult * metrics_dict[f"distortion_{lod}"]
                )
            ...
            if self.config.sparsity_loss_mult > 0.0:
                ...
                random_positions = -1.0 + 2.0 * torch.rand(num_random_samples, 3).to(
                    self.device
                )

                # random_positions = WORLD_MIN + (WORLD_MAX - WORLD_MIN) * torch.rand(num_random_samples, 3).to(self.device)
                random_viewdirs = torch.normal(
                    mean=0, std=1, size=(num_random_samples, 3)
                ).to(self.device)
                ...
                for lod in range(self.config.num_lods):
                    ...
                    for chunk_model in self._model[lod]:
                        density = chunk_model.field(
                            None, random_viewdirs, random_positions
                        )[FieldHeadNames.DENSITY]
                        loss_dict[f"sparsity_loss_{lod}"] += (
                            self.config.sparsity_loss_mult
                            * sparsity_loss(
                                random_positions,
                                random_viewdirs,
                                density,
                                chunk_model.voxel_size_to_use,
                            )
                        )

            if self.config.accumulation_loss_mult > 0.0:
                for lod in range(self.config.num_lods):
                    loss_dict[f"acc_loss_{lod}"] = (
                        self.config.accumulation_loss_mult
                        * self.acc_loss(
                            outputs["accumulation"][lod].clip(1e-5, 1.0 - 1e-5),
                            torch.ones_like(outputs["accumulation"][lod]),
                        )
                    )
    ...
    def get_image_metrics_and_images(
        ...
        for lod in range(self.config.num_lods):
            ...
            combined_rgb = torch.cat(
                [image.to(outputs[f"rgb_{lod}"].device), rgb], dim=1
            )
            ...
            metrics_dict.update(
                {
                    f"psnr_{lod}": float(psnr.item()),
                    f"ssim_{lod}": float(ssim),
                    f"lpips_{lod}": float(lpips),
                }
            )  # type: ignore
            ...
            images_dict.update(
                {
                    f"img_{lod}": combined_rgb,
                    f"accumulation_{lod}": combined_acc,
                    f"depth_{lod}": combined_depth,
                }
            )
        ...
        for lod in range(self.config.num_lods):
            for i in range(0, num_rays, num_rays_per_chunk):
                ...
                for output_name, output in outputs.items():  # type: ignore
                    if (
                        output_name == "rgb"
                        or output_name == "depth"
                        or output_name == "accumulation"
                        or output_name == "rgb_global"
                    ):
            ...
            for output_name, outputs_list in outputs_lists.items():
                if self.config.global_deferred:
                    if (
                        ...
                        outputs[output_name + f"_{lod}"] = torch.cat(outputs_list).view(
