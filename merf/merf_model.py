...
class MERFHashMLPDensityField(Field):
    ...
    def get_density(
        self, ray_samples: RaySamples, samples_transform: Tensor = None
    ) -> Tuple[Tensor, None]:
        if self.spatial_distortion is not None:
            ...
            if samples_transform is not None:
                positions = torch.matmul(
                    samples_transform,
                    torch.cat(
                        [positions, torch.ones(*positions.shape[:-1], 1).to(positions)],
                        dim=-1,
                    ).T,
                ).T[:, 0:3]
            ...
            if self.gaussian_samples_in_prop:
                positions = SceneBox.get_normalized_positions(
                    ray_samples.frustums.get_gaussian_samples(), self.aabb
                )
            else:
                positions = SceneBox.get_normalized_positions(
                    ray_samples.frustums.get_gaussian_samples(), self.aabb
        ...
        if not self.use_linear:
            ...
            density_before_activation = (
                self.mlp_network(hash_features)
...
class GradientScaler(torch.autograd.Function):
    ...
    def backward(ctx, grad_output_colors, grad_output_sigmas, grad_output_ray_dist):
        ...
        return (
            grad_output_colors * scaling,
            grad_output_sigmas * scaling,
...
def conical_frustum_to_gaussian_mean(
    ...
    return origins + directions * (
        mu + (2.0 * mu * hw * hw) / (3.0 * mu * mu + hw * hw)
    )
...
class FeatureRenderer(nn.Module):
    ...
    def combine_rgb(
        ...
        if background_color == "random":
            background_color = torch.rand_like(comp_features[Ellipsis, 0:3]).to(
                features.device
            )
        ...
        comp_diffuse_rgb = comp_features[Ellipsis, 0:3] + background_color.to(
            weights.device
...
class MERFModel(Model):
    ...
    def populate_modules(self):
        ...
        if (
            self.config.proposal_initial_sampler == "uniform"
            or (not self.config.spatial_distortion)
            or (self.config.bounded_scene)
        ):
    ...
    def get_training_callbacks(
        ...
        def set_sparsity_loss_mult(step):
            ...
            if step < self.config.sparsity_loss_mult[0]:
                ...
                t = (step - self.config.sparsity_loss_mult[0]) / (
                    self.config.sparsity_loss_mult[1]
                    - self.config.sparsity_loss_mult[0]
                )
                print(
                    exp_lerp(
                        t,
                        self.config.sparsity_loss_mult[2],
                        self.config.sparsity_loss_mult[3],
                    )
                )
                self.sparsity_loss_mult = exp_lerp(
                    t,
                    self.config.sparsity_loss_mult[2],
                    self.config.sparsity_loss_mult[3],
                )
    ...
    def get_baking_outputs_for_camera_ray_bundle(self, ray_bundle: RayBundle):
        ...
        field_outputs = self.field.get_outputs(
            ray_samples, ray_samples.frustums.directions
        )
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        features = self.renderer_features(
            features=field_outputs["features"], weights=weights
        )
        ...
        for name, param in self.named_parameters():
            ...
            for key in self.config.param_regularizers:
                if key in name:
                    ...
                    loss_reg += mult * acc_fn(
                        lossfun(param, torch.tensor(alpha), torch.tensor(scale))
                    )
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        if self.training:
            ...
            if self.sparsity_loss_mult > 0.0:
                ...
                if self.config.bounded_scene:
                    random_positions = self.scene_box.aabb[0].min() + (
                        self.scene_box.aabb[1].max() - self.scene_box.aabb[0].min()
                    ) * torch.rand(num_random_samples, 3).to(self.device)
                else:
                    random_positions = WORLD_MIN + (WORLD_MAX - WORLD_MIN) * torch.rand(
                ...
                density = self.field(None, random_viewdirs, random_positions)[
                    FieldHeadNames.DENSITY
                ]
                loss_dict["sparsity_loss"] = self.sparsity_loss_mult * sparsity_loss(
                    random_positions, random_viewdirs, density, self.voxel_size_to_use
                )
        ...
        with torch.no_grad():
            ...
            with Progress(
                ...
                task = progress.add_task(
                    "[green]Evaluating all images for baking...", total=num_images
                )
                ...
                for (
                    ...
                    num_batch = (
                        ...
                        // self.config.baking_config.eval_chunk
                    )
                    for i in range(num_batch):
                        if i < num_batch - 1:
                            batch_camera_ray_bundle = camera_ray_bundle[
                                i * self.config.baking_config.eval_chunk : i
                                * self.config.baking_config.eval_chunk
                                + self.config.baking_config.eval_chunk
                            ]
                        else:
                            batch_camera_ray_bundle = camera_ray_bundle[
                                (num_batch - 1) * self.config.baking_config.eval_chunk :
                            ]
                        if chunks_model is not None:
                            outputs = (
                                chunks_model.get_baking_outputs_for_camera_ray_bundle(
                                    self.collider(batch_camera_ray_bundle),
                                    lod_idx=lod_idx,
                                    chunk_idx=chunk_idx,
                                )
                            )  ##TODO collider fixed
                        else:
                            outputs = self.get_baking_outputs_for_camera_ray_bundle(
                                self.collider(batch_camera_ray_bundle)
                            )
                        if outputs is None:
                            continue
                        ...
                        if chunks_model is not None:
                            positions = torch.matmul(
                                chunks_model.chunks_transform[lod_idx][chunk_idx],
                                torch.cat(
                                    [
                                        positions,
                                        torch.ones(*positions.shape[:-1], 1).to(
                                            positions
                                        ),
                                    ],
                                    dim=-1,
                        ...
                        alive_samples = (weights > weight_threshold).squeeze(
                            -1
                        )  # BxS.``
                        ...
                        if self.config.baking_config.use_alpha_culling:
                            ...
                            alpha = 1.0 - trunc_exp(
                                -density_alive.to(self.device) * step_sizes
                            )
                        ...
                        def remove_out_of_bound_points(x):
                            mask = (x >= 0).all(dim=1) & (
                                x < self.grid_config["resolution_to_use"]
                            ).all(dim=1)
                            return x[mask]
                        ...
                        for z in itertools.product(*corner_coords):
                            ...
                            for i, b in enumerate(z):
                                l.append(
                                    positions_alive_1[..., i]
                                    if b
                                    else positions_alive_0[..., i]
                                )
                            positions_corner = torch.stack(l, dim=-1)

                            positions_corner = remove_out_of_bound_points(
                                positions_corner
                            )
    ...
    def bake_sparse_grid(self, alive_macroblocks):
        ...
        for block_start in tqdm.tqdm(
            ...
            features, density = self.field.evaluate_to_baking(
                positions
            )  # BxAxAxAx7, BxAxAxAx3.
    ...
    def export_scene(
        ...
        for plane_idx, (plane_features, plane_density) in enumerate(
            ...
            output_paths.append(path.join(baked_dir, f"plane_features_{plane_idx}.png"))
        ...
        for occupancy_grid_factor, occupancy_grid in occupancy_grids:
            ...
            occupany_grid_image = (
                np.transpose(
                    np.repeat(occupancy_grid[Ellipsis, None], 4, -1), [2, 1, 0, 3]
                )
        ...
        export_scene_params.update(
            {
                "0_weights": self.deferred_mlp.layers[0]
                .weight.data.cpu()
                .transpose(0, 1)
                .tolist(),
                "1_weights": self.deferred_mlp.layers[1]
                .weight.data.cpu()
                .transpose(0, 1)
                .tolist(),
                "2_weights": self.deferred_mlp.layers[2]
                .weight.data.cpu()
                .transpose(0, 1)
                .tolist(),
    ...
    def baking_merf_model(
        self, datamanager, chunks_model=None, lod_idx=None, chunk_idx=None
    ):
        assert self.config.baking_config is not None
        with torch.no_grad():
            gc.collect()
            planes_features = planes_density = sparse_grid_features = (
                sparse_grid_density
            ) = sparse_grid_block_indices = None
            self.grid_config = calculate_grid_config(self.config)
            ...
            masks_2d_list_path = []
            alive_voxels_path = path.join(
                self.config.baking_config.baking_path, "alive_voxels.npy"
            )
            for i in range(5):
                occupancy_grids_path.append(
                    path.join(
                        self.config.baking_config.baking_path,
                        f"occupancy_grids_{i}.npy",
                    )
                )
            for i in range(3):
                masks_2d_list_path.append(
                    path.join(
                        self.config.baking_config.baking_path, f"masks_2d_list_{i}.npy"
                    )
                )

            alive_voxels_3d_grid_path = path.join(
                self.config.baking_config.baking_path, "alive_voxels_3d_grid.npy"
            )
            ...
            if self.config.baking_config.load_alive_voxels_from_disk:
                if self.config.baking_config.load_occ_grid:
                    ...
                    masks_2d_list = []
                    for i in range(5):
                        occupancy_grids.append(
                            (
                                occupancy_grid_factors[i],
                                np.load(occupancy_grids_path[i]),
                            )
                        )
                    for i in range(3):
                ...
                alive_voxels = self.compute_alive_voxels(
                    chunks_model=chunks_model, lod_idx=lod_idx, chunk_idx=chunk_idx
                )
            ...
            if (
                self.config.baking_config.save_alive_voxels_to_disk
                and not self.config.baking_config.load_alive_voxels_from_disk
            ):
            ...
            if not self.config.baking_config.load_occ_grid:
                ...
                for i, occupancy_grid_factor in enumerate(occupancy_grid_factors):
                    occupancy_grid = skimage.measure.block_reduce(
                        occupancy_grid,
                        (
                            occupancy_grid_factor // pre_factor,
                            occupancy_grid_factor // pre_factor,
                            occupancy_grid_factor // pre_factor,
                        ),
                        np.max,
                    ...
                    print(
                        f"occupancy_grid_factor_{occupancy_grid_factor} has been computed"
                    )
            ...
            # Baking sparse grid
            batch_size_in_blocks = self.config.baking_config.batch_size_in_blocks
            sparse_grid_features_1d, sparse_grid_density_1d = self.bake_sparse_grid(
                alive_macroblocks
            )
            ...
            vram_consumption = {
                "sparse_3d_grid": as_mib(sparse_grid_features)
                + as_mib(sparse_grid_density),
            ...
            save_json(
                vram_consumption,
                os.path.join(self.config.baking_config.baking_path, "vram.json"),
            )

            planes_features, planes_density = map_quantize_tuple_list(
                planes_features,
                planes_density,
            )

            sparse_grid_features, sparse_grid_density = map_quantize_tuple(
                sparse_grid_features,
                sparse_grid_density,
            )
