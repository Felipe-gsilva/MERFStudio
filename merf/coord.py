...
def piecewise_warp_fwd(x, eps=torch.finfo(torch.float32).eps):
    ...
    return torch.where(
        x < 1, 0.5 * x, 1 - 0.5 / torch.maximum(torch.ones_like(x) * eps, x)
    )
...
def piecewise_warp_inv(x, eps=torch.finfo(torch.float32).eps):
    ...
    return torch.where(
        x < 0.5, 2 * x, 0.5 / torch.maximum(torch.ones_like(x) * eps, 1 - x)
    )
...
def contract(x):
    ...
    x_max = torch.maximum(
        1.0 * torch.ones_like(x_abs), torch.amax(x_abs, dim=-1, keepdims=True)
    )
