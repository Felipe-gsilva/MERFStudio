...
def simulate_alpha_culling(
    ...
    def zero_density_below_threshold(density):
        if viewdirs.shape != positions.shape:
            viewdirs_b = torch.broadcast_to(viewdirs, positions.shape).reshape(-1, 3)
        else:
...
class MERFViewEncoding(Encoding):
    ...
    def get_out_dim(self) -> int:
        ...
        return (
            self.in_dim * self.deg_enc * 2 + self.in_dim
            if self.include_input
            else self.in_dim * self.deg_enc * 2
        )
    ...
    def forward(
        ...
        return pos_enc(
            in_tensor,
            min_deg=0,
            max_deg=self.deg_enc,
            append_identity=self.include_input,
        )
...
class MERFactoField(Field):
    ...
    def evaluate_to_baking(
        self, positions: Tensor, appearance_embedding=None
    ) -> Tuple[Tensor, Tensor]:
        ...
        if self.use_appearance_embedding or appearance_embedding is not None:
            embedded_appearance = torch.ones(
                (*tgpositions.shape[:-1], self.appearance_embedding_dim),
                device=tgpositions.device,
    ...
    def get_outputs(
        ...
        viewdirs: Optional[Tensor] = None,
        positions=None,
        samples_transform=None,
                ...
                if samples_transform is not None:
                    positions = torch.matmul(
                        ...
                        torch.cat(
                            [
                                positions,
                                torch.ones(*positions.shape[:-1], 1).to(positions),
                            ],
                            dim=-1,
                        ).T,
                    ).T[:, 0:3]
        ...
        if self.use_appearance_embedding or appearance_embedding is not None:
            if self.training and not only_return_density:
                embedded_appearance = appearance_embedding(camera_indices).view(
            ...
            density = simulate_quantization(
                density,
                self.merf_model_config.range_density[0],
                self.merf_model_config.range_density[1],
            )
            ...
            features = simulate_quantization(
                features,
                self.merf_model_config.range_features[0],
                self.merf_model_config.range_features[1],
            )
    ...
    def forward(
        self, ray_samples: RaySamples, viewdirs: Optional[Tensor] = None, positions=None
    ) -> Dict[FieldHeadNames, Tensor]:
        ...
        field_outputs = self.get_outputs(
            ray_samples, viewdirs=viewdirs, positions=positions
        )
