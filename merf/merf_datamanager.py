...
from merf.point_util.visualize_pointcloud_and_camera import (
    visualize_pointcloud_and_cameras,
    visualize_camera_poses_plotly,
)
import os
import warnings
...
def align_rectangle_with_axes(input_poses):
    ...
    aligned_poses[:, :3, :3] = np.einsum(
        "ij,njk->nik", rot_matrix, aligned_poses[:, :3, :3]
    )
    aligned_poses[:, :3, 3] = np.einsum(
        "ij,nj->ni", rot_matrix, aligned_poses[:, :3, 3]
    )

    return torch.tensor(aligned_poses)


def get_train_eval_split_blender(
...
class MerfDataParser(Nerfstudio):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for frame in frames:
            ...
            if self.config.eval_mode == "blender":
                image_filenames.append(
                    fname
                    if "_eval" not in str(fname)
                    else Path(str(fname).replace("_eval", ""))
                )
        ...
        if self.config.is_transform:
            ...
            self.padding = torch.tensor([[0.0, 0.0, 0.0, 1.0]], dtype=torch.float32)
            transform_final = torch.diag(
                torch.tensor([scale_factor] * 3 + [1])
            ) @ torch.cat([transform_matrix, self.padding], dim=0)
            ...
            self.center, self.scene_scale_t, maxz, minz = get_center_and_scale(
                out_path_
            )
            ...
            points -= self.center
            points *= 1.0 / self.scene_scale_t
            poses_[:, :3, 3] -= torch.tensor(
                self.center, dtype=torch.float32
            )  # transform
            poses_[:, :3, 3] *= torch.tensor(
                1.0 / self.scene_scale_t, dtype=torch.float32
            )  # scale
            poses = poses_[:, :3, :]
        ...
        visualize_pointcloud_and_cameras(
            np.array([[0.0, 0.0, 0.0]]), poses[:, :3, 3].cpu().numpy()
        )
        ...
        scene_box = SceneBox(
            aabb=torch.tensor(
                [[-aabb_scale, -aabb_scale, minz], [aabb_scale, aabb_scale, maxz]],
                dtype=torch.float32,
            )
        )
