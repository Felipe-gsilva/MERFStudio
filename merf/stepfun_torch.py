...
def sorted_interp(x, xp, fp, eps=torch.finfo(torch.float32).eps ** 2):
    """A version of interp() where xp and fp must be sorted."""
    (xp0, xp1), (fp0, fp1) = sorted_lookup(x, xp, (xp, fp))[1]
    offset = torch.clip(
        (x - xp0) / torch.maximum(torch.ones_like(xp1) * eps, xp1 - xp0), 0, 1
    )
...
def weight_to_pdf(t, w, eps=torch.finfo(torch.float32).eps ** 2):
    ...
    return w / torch.maximum(
        torch.ones_like(t[Ellipsis, 1:]) * eps, (t[Ellipsis, 1:] - t[Ellipsis, :-1])
    )
...
def max_dilate_weights(
    ...
    if renormalize:
        w_dilate /= torch.maximum(
            torch.ones_like(w_dilate) * eps, torch.sum(w_dilate, dim=-1, keepdims=True)
        )
    return t_dilate, w_dilate
...
def integrate_weights(w):
    ...
    cw0 = torch.concat(
        [torch.zeros(shape).to(cw.device), cw, torch.ones(shape).to(cw.device)], dim=-1
    )
    return cw0
...
def sample_intervals(
    ...
    centers = sample(t, w_logits, num_samples, single_jitter, deterministic_center=True)
    ...
    first = torch.maximum(
        minval * torch.ones_like(centers[Ellipsis, :1]),
        2 * centers[Ellipsis, :1] - mid[Ellipsis, :1],
    )
    last = torch.minimum(
        maxval * torch.ones_like(centers[Ellipsis, :1]),
        2 * centers[Ellipsis, -1:] - mid[Ellipsis, -1:],
    )
